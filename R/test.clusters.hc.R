...
#' @param plot Whether to plot the dendogram retrieved by the hierarchical clustering algorithm
...
#' test.clusters.hc(X, U, Sigma = NULL, Y = Y, NC = 3, 
#' clusters = sample(1:3, 2), plot = TRUE, linkage = "complete")
#'
...
test.clusters.hc <- function(X, U = NULL, Sigma = NULL, Y = NULL, UY = NULL, precUY = NULL, NC, clusters, linkage = 'average', plot = FALSE, ndraws = 2000){
  ...
  if(plot){ # Plot dendogram
    
    # Code from clusterpval package (Gao et al. 2022)
    plot(stats::as.dendrogram(hcl), leaflab = "none")
    graphics::abline(h = (hcl$height[nrow(X) - NC]), lty = "dashed", col = "darkgrey")
    clusterpval::rect_hier_clusters(hcl, k = NC, which = 1:NC, border = RColorBrewer::brewer.pal(NC, "Dark2"))
  ...
  diff_means <- Matrix::Matrix(colMeans(X[hcl_at_K == clusters[1], , drop = FALSE]) - colMeans(X[hcl_at_K == clusters[2], , drop = FALSE]), sparse = TRUE) 
  ...
  U_g1g2 <- U[hcl_at_K == clusters[1] | hcl_at_K == clusters[2], hcl_at_K == clusters[1] | hcl_at_K == clusters[2]]
  ...
  if(!linkage == "complete"){ # Code adapted from clusterpval package (Gao et al. 2022)
    ...
    S2 <- test_gao$trunc # Truncation set for the 2-norm
