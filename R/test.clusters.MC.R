...
#'   \item{X_clus}{If return_X_clus = TRUE and sample_split = TRUE, the data matrix used for clustering/testing (i.e., the subsample of \code{X} retained for the clustering step).}
#' }
#'
#' @details
...
test.clusters.MC <- function(X, U = NULL, Sigma = NULL, Y = NULL, UY = NULL, precUY = NULL, clusters, cl_fun, NC = NULL, cl = NULL, ndraws = 2000, sample_split = FALSE, nY = NULL, return_Sigma = FALSE, return_X_clus = FALSE){
  ...
  if(survives == 0) {
    ...
    return(list(stat = stat_V, pvalue = NA, stderr = NA, clusters = cl))
  }
