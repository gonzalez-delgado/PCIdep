...
where $H_0^{\{C_1,C_2\}}$ is the null hypothesis of equality of clusters, that needs to be precised for a given data model.
...
<div style="border-left: 4px solid #e6a817; background-color: #fdf6e3; padding: 12px 16px; margin: 16px 0; border-radius: 4px;">
  ...
  present substantial deviations from the compound symmetry class (e.g. for auto-regressive processes).
...
- For hierarchical agglomerative clustering (HAC) and $k$-means, see [the HAC and $k$-means vignette](hac-km-clustering.Rmd).
- For user-defined or custom clustering procedures, see [the custom clustering vignette](custom-clustering.Rmd).
