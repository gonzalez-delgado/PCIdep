...
where $`\mathbf{\Sigma}\in\mathcal{M}_{n\times p}(\mathbb{R})`$ and $`\mathbf{\Sigma}\in\mathcal{M}_{n\times p}(\mathbb{R})`$ are positive definite matrices encoding the depence structure between observations and features respectively.
